...
pathbnmask = 'samba/actpol/tilec/v1.2.0_20200324/map_v1.2.0_joint_boss/tilec_mask.fits' #Mask for the BN ILC area
pathd56mask = 'samba/actpol/tilec/v1.2.0_20200324/map_v1.2.0_joint_deep56/tilec_mask.fits' #Mask for the D56 ILC area
V20old= pd.read_csv('V20_DR15_Catalog_v2.csv',comment='#') #Load old version of catalog if needed to make edits

#Define arrays for catalog (load by column) 
...
#hmap_50=hp.read_map('product-action?MAP.MAP_ID=COM_CompMap_Compton-SZMap-masks_2048_R2.01.fits',field=1)
#
#coord_icrs=SkyCoord(ra=ra*u.degree,dec=dec*u.degree) #establish coordinates
#coord_gal=coord_icrs.galactic
#
#hpixels=hel.lonlat_to_healpix(coord_gal.l.degree*u.deg,coord_gal.b.degree*u.deg) #change coordinate units 
...
#
#galcut=np.zeros(len(lum)) #initialize arrays
#galcut40=np.zeros(len(lum))
#galcut50=np.zeros(len(lum))
#
#for i in range(len(lum)): #make cuts
#        if values_40[i]>0.99:
#		galcut40[i]=1
#        if values_50[i]>0.99:
#                galcut50[i]=1
...
#                galcut[i]=2
#
#print len([x for x in galcut if x==0]) #print the number of sources per cut bin
...
for i in range(len(z)):
	if (PS15mJy_cut[i]==1 and PS100mJy_cut[i]==1 and divcuts18[i]==2 and galcut[i]==2):
		S18coadd[i]=1
	if (PS15mJy_cut[i]==1 and PS100mJy_cut[i]==1 and divcut[i]==2 and galcut[i]==2 and (incompsepregion[i]==1 or incompsepregion[i]==2)):
		S16ILC[i]=1

#Printing out reference values with luminosity bins 
print 'Total length S18 Coadd=',len([x for x in S18coadd if x==1])
print 'Total length S16 ILC=',len([x for x in S16ILC if x==1])

print 'avg mass full sample s18coadd',np.mean([halomasses[x] for x in range(len(lum)) if S18coadd[x]==1 and lum[x]>4.3e10])
print 'avg stellar mass full sample s18coadd',np.mean([stellarmasses[x] for x in range(len(lum)) if S18coadd[x]==1 and lum[x]>4.3e10])

print 'avg mass full sample s18coadd',np.mean([halomasses[x] for x in range(len(lum)) if S16ILC[x]==1 and lum[x]>4.3e10])
print 'avg stellar mass full sample s18coadd',np.mean([stellarmasses[x] for x in range(len(lum)) if S16ILC[x]==1 and lum[x]>4.3e10])

print 'Bins for coadds:'
print len([x for x in range(len(lum)) if S18coadd[x]==1 and lum[x]>4.3e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and lum[x]>6.1e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and lum[x]>7.9e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and lum[x]>9.8e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and lum[x]>11.6e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and 4.3e10<=lum[x]<6.10e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and 6.1e10<=lum[x]<7.90e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and 7.9e10<=lum[x]<9.80e10])
print len([x for x in range(len(lum)) if S18coadd[x]==1 and 9.8e10<=lum[x]<11.60e10])


print 'Bins for ILC:'
print len([x for x in range(len(lum)) if S16ILC[x]==1 and lum[x]>4.3e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and lum[x]>6.1e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and lum[x]>7.9e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and lum[x]>9.8e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and lum[x]>11.6e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and 4.3e10<=lum[x]<6.10e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and 6.1e10<=lum[x]<7.90e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and 7.9e10<=lum[x]<9.80e10])
print len([x for x in range(len(lum)) if S16ILC[x]==1 and 9.8e10<=lum[x]<11.60e10])


#Save the new catalog 
np.savetxt('V20_DR15_Catalog_v3.csv',np.array([ra,dec,lum,z,stellarmasses,halomasses,cModelMag_u,cModelMag_g,cModelMag_r,cModelMag_i,cModelMag_z,cModelMagErr_u,cModelMagErr_g,cModelMagErr_r,cModelMagErr_i,cModelMagErr_z,petroMag_u,petroMag_g,petroMag_r,petroMag_i,petroMag_z,petroMagErr_u,petroMagErr_g,petroMagErr_r,petroMagErr_i,petroMagErr_z,extinction_u,extinction_g,extinction_r,extinction_i,extinction_z,bestObjID,PS15mJy_cut,PS100mJy_cut,divcut,divcuts18,galcut,incompsepregion,S18coadd,S16ILC]).T,delimiter=',',fmt="%s") 
